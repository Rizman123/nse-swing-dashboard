...
REGIME_CONTEXT_NOTE = {
    "RISK-ON": "RISK-ON regime - broad market participation supports follow-through on breakouts.",
    "NEUTRAL": "NEUTRAL regime - selective, high-conviction setups only, size positions carefully.",
    "RISK-OFF": "RISK-OFF regime - every score already reduced by 20 points; treat all setups with extra caution.",
}


def build_summary_points(checklist, market_regime):
    """Structured, deterministic breakdown for the detail panel - one labeled
    point per pillar plus a lead signal, a key risk, and market context.
    Never depends on Gemini, so it's always available and always accurate."""
    by_category = {}
    order = []
    ...
    fired = [c for c in checklist if c["status"] == "pass"]
    not_fired = [c for c in checklist if c["status"] == "fail"]
    ...
    if fired:
        ...
        points.append({
            ...
            "text": f"{lead['signal']} - actual {lead['actual']} (needs {lead['threshold']}).",
        })
    else:
        points.append({
            "label": "Primary signal",
            "text": "No signals have fired yet - this is a watchlist name, not a confirmed setup.",
        })
    ...
    for category in order:
        ...
        points.append({
            "label": category,
            "text": f"{passed} of {len(items)} signals confirmed.",
        })

    if not_fired:
        risk = not_fired[0]
        points.append({
            "label": "Key risk to watch",
            "text": f"{risk['signal']} has not confirmed - actual {risk['actual']}, needs {risk['threshold']}.",
        })
    else:
        points.append({
            "label": "Key risk to watch",
            "text": "All 15 signals have confirmed - unusually strong setup, still size and stop-loss appropriately.",
        })

    points.append({"label": "Market context", "text": REGIME_CONTEXT_NOTE.get(market_regime, "")})
...
def _resolve_gemini_model(genai):
    """Model names get renamed/retired by Google over time (e.g. gemini-1.5-flash
    was retired). Instead of hardcoding one and breaking silently, ask Gemini
    what's actually available and pick a fast, current model. Cached per process
    so this only happens once, not on every request."""
    global _GEMINI_MODEL_NAME
    if _GEMINI_MODEL_NAME:
        return _GEMINI_MODEL_NAME

    # Names to avoid even though they technically support generateContent -
    # these are audio/TTS/image/embedding variants, not plain text chat models.
    exclude_terms = ("tts", "audio", "image", "embedding", "vision", "live", "native")
    ...
    if not candidates:
        raise RuntimeError("No plain text-generation Gemini models available for this API key")

    # Prefer a "flash" model (cheap + fast) over "pro". Among those, the
    # shortest name reliably picks the plain stable release (e.g.
    # "models/gemini-2.5-flash") over dated/preview variants with extra
    # "-preview-..." suffixes tacked on.
    flash_models = [c for c in candidates if "flash" in c.lower()]
    pool = flash_models if flash_models else candidates
    _GEMINI_MODEL_NAME = min(pool, key=len)
    print(f"[INFO] Using Gemini model: {_GEMINI_MODEL_NAME}")
    return _GEMINI_MODEL_NAME
...
def dashboard_payload():
    ...
    for s, score, checklist, fired_names in top5:
        ...
        summary_points = build_summary_points(checklist, market["regime"])
        top_5_stocks.append({
            "symbol": s["symbol"],
...
# ---------------------------------------------------------------------------
# Daily Zerodha login. Nobody's password ever touches this server - your dad
# logs in directly on Zerodha's own site with his OTP, and Zerodha hands back
# a one-day access token. This just automates opening that login page and
# reading the token back, same idea as a local batch file, but as a webpage.
# ---------------------------------------------------------------------------

@app.get("/zerodha/login")
def zerodha_login():
    if not ZERODHA_API_KEY:
        return "ZERODHA_API_KEY is not set on the server yet - add it on Render first.", 500
...
def zerodha_callback():
    ...
    if not request_token:
        return render_template("zerodha_callback.html", error="No login code came back from Zerodha. Please try logging in again.", access_token=None)
    if not (ZERODHA_API_KEY and ZERODHA_API_SECRET):
        return render_template("zerodha_callback.html", error="ZERODHA_API_KEY / ZERODHA_API_SECRET are not set on the server yet.", access_token=None)
    try:
        from kiteconnect import KiteConnect
        kite = KiteConnect(api_key=ZERODHA_API_KEY)
        session_data = kite.generate_session(request_token, api_secret=ZERODHA_API_SECRET)
        access_token = session_data["access_token"]
        return render_template("zerodha_callback.html", error=None, access_token=access_token)
    except Exception as exc:
        return render_template("zerodha_callback.html", error=str(exc), access_token=None)
